...
<body>
	...
	<% MemberBean r = (MemberBean)session.getAttribute("member"); %>
	<!-- 登録画面作成 -->
	<div class="mainContent">
   	<form action = "update3" method = "post">
   	<input type="hidden" name="origin_id" value="<%=r.getMember_id() %>">
    <h2>会員情報登録</h2>
    <p>■ID<span class="req">変更不可</span></p>
    <input type= "text" name = "member_id" maxlength="20" required = "required"  pattern="^[a-zA-Z0-9]+$" placeholder="※20文字以内の半角英数字" value = "<%=r.getMember_id() %>" readonly>
    <p>■パスワード<span class="req">必須</span></p>
    <input type= "text" name = "password" maxlength="20" required = "required"  pattern="^[a-zA-Z0-9]+$" placeholder="※20文字以内の半角英数字" value = "<%=r.getPassword() %>">
